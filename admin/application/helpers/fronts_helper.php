...
function vartransactions()
{
	$arr = array("Waiting","Process","Failed");
	return $arr;
}
function transactions($no = "")
{
	$xx = vartransactions();
	return isset($xx[$no])?$xx[$no]:"";		
}
function varcolortransactions()
{
	$arr = array("black","blue","red","yellow");	
	return $arr;
}
function colotransactions($no = "")
{
	$xx = varcolorpayment();
	return isset($xx[$no])?$xx[$no]:"";		
}
