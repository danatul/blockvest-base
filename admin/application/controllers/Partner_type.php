...
class Partner_type extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		$in['use_hedaer'] = true;
		$in['title'] = 'Partner Type';
		$in['desc'] = 'you can manage your Partner Type here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/partner_type')] = 'Partner Type';
		$in['tpl'] = 'partner_type/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_partner_type.id';
			$columns    = array(
			array('db'=>'m_partner_type.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'m_partner_type.name','dt'=>1,'alias'=>'partner_type','formatter'=>function($d,$row){
				return $d;
			}), 
			 
			array('db'=>"m_partner_type.displays",'dt'=>2,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_partner_type.id','dt'=>3,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('partner_type',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('partner_type');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
					...
					$in['updated_on'] = $time;
					$this->db->where('id',$in['id']);
					$this->db->update('partner_type',$in);
				}
				else
				{
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('partner_type',array("displays"=>$in['displays']));
