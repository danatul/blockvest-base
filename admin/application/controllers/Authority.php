...
class Authority extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		$in['use_hedaer'] = true;
		$in['title'] = 'Authority';
		$in['desc'] = 'you can manage your Authority here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/authority')] = 'Authority';
		$in['tpl'] = 'authority/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_authority.id';
			$columns    = array(
			array('db'=>'m_authority.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			array('db'=>"m_authority.image",'dt'=>1,'alias'=>'image','formatter'=>function($d,$row){
			...
			array('db'=>'m_authority.name','dt'=>2,'alias'=>'name','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_authority.links','dt'=>3,'alias'=>'type','formatter'=>function($d,$row){
				return '<a href="'.$d.'" target="_blank">Links</a>';
			}), 
			array('db'=>'m_authority.email','dt'=>4,'alias'=>'email','formatter'=>function($d,$row){
				return $d;
			}),
			array('db'=>'m_authority.address','dt'=>5,'alias'=>'address','formatter'=>function($d,$row){
				return $d;
			}), 
			 
			array('db'=>"m_authority.displays",'dt'=>6,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_authority.id','dt'=>7,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function delete_logo()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->update('authority',array('image'=>''));
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('authority',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('authority');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('authority',array("displays"=>$in['displays']));
