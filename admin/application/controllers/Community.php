...
class Community extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		$in['bread'][site_url('manager/community')] = 'Portal';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_community.id';
			$columns    = array(
			array('db'=>'m_community.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			 
			array('db'=>'m_community.images','dt'=>1,'alias'=>'images','formatter'=>function($d,$row){
			...
			array('db'=>'m_community.name','dt'=>2,'alias'=>'name','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_community.telegram','dt'=>3,'alias'=>'company','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_community.pic','dt'=>4,'alias'=>'nama','formatter'=>function($d,$row){
				 return $d;
			}), 
			array('db'=>'m_community.member','dt'=>5,'alias'=>'designation','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>"m_community.displays",'dt'=>6,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_community.id','dt'=>7,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('community',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('community');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('community',array("displays"=>$in['displays']));
