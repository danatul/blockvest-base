...
class Influencer extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		$in['bread'][''] = 'Influencer';
		$in['bread'][site_url('manager/influencer')] = 'Authority';
		$in['tpl'] = 'influencer/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_influencer.id';
			$columns    = array(
			array('db'=>'m_influencer.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'m_influencer.channel_name','dt'=>1,'alias'=>'name','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_influencer.channel_id','dt'=>2,'alias'=>'channel_id','formatter'=>function($d,$row){
				return '<a href="https://www.youtube.com/channel/'.$d.'" target="_blank">Links</a>';
			}), 
			 
			array('db'=>"m_influencer.displays",'dt'=>3,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_influencer.id','dt'=>4,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function delete_logo()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->update('influencer',array('image'=>''));
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('influencer',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('influencer');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('influencer',array("displays"=>$in['displays']));
