...
class Kol_category extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		$in['use_hedaer'] = true;
		$in['title'] = 'KOL Category';
		$in['desc'] = 'you can manage your KOL Category here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/kol_category')] = 'KOL Category';
		$in['tpl'] = 'kol_category/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_kol_category.id';
			$columns    = array(
			array('db'=>'m_kol_category.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'m_kol_category.name','dt'=>1,'alias'=>'kol_category','formatter'=>function($d,$row){
				return $d;
			}), 
			 
			array('db'=>"m_kol_category.displays",'dt'=>2,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_kol_category.id','dt'=>3,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('kol_category',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('kol_category');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
					...
					$in['updated_on'] = $time;
					$this->db->where('id',$in['id']);
					$this->db->update('kol_category',$in);
				}
				else
				{
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('kol_category',array("displays"=>$in['displays']));
