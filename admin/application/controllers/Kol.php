...
class Kol extends Smart_Controller
{
	private $_users;
	...
	{
		$in['kol_category'] = $this->db->get('kol_category')->result_array(); 
		$in['use_hedaer'] = true;
		$in['title'] = 'Speaker';
		$in['desc'] = 'you can manage your Portal here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/kol')] = 'Portal';
		$in['tpl'] = 'kol/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$primaryKey = 'm_kol.id';
			$columns    = array(
			array('db'=>'m_kol.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'(select a.name from m_kol_category a where a.id=m_kol.id_kol_category limit 1)','dt'=>1,'alias'=>'cats','formatter'=>function($d,$row){
				return $d;
			}),
			array('db'=>'m_kol.images','dt'=>2,'alias'=>'images','formatter'=>function($d,$row){
			...
			array('db'=>'m_kol.company','dt'=>3,'alias'=>'company','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_kol.nama','dt'=>4,'alias'=>'nama','formatter'=>function($d,$row){
				 return $d;
			}), 
			array('db'=>'m_kol.designation','dt'=>5,'alias'=>'designation','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>"m_kol.displays",'dt'=>6,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_kol.id','dt'=>7,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('kol',array("displays"=>$in['displays']));
