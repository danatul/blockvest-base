...
class Speaker extends Smart_Controller
{
	private $_users;
	...
	{
		$in['speaker_category'] = $this->db->get('speaker_category')->result_array(); 
		$in['use_hedaer'] = true;
		$in['title'] = 'Speaker';
		$in['desc'] = 'you can manage your Portal here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/speaker')] = 'Portal';
		$in['tpl'] = 'speaker/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_speaker.id';
			$columns    = array(
			array('db'=>'m_speaker.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'(select a.name from m_speaker_category a where a.id=m_speaker.id_speaker_category limit 1)','dt'=>1,'alias'=>'cats','formatter'=>function($d,$row){
				return $d;
			}),
			array('db'=>'m_speaker.images','dt'=>2,'alias'=>'images','formatter'=>function($d,$row){
			...
			array('db'=>'m_speaker.company','dt'=>3,'alias'=>'company','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>'m_speaker.nama','dt'=>4,'alias'=>'nama','formatter'=>function($d,$row){
				 return $d;
			}), 
			array('db'=>'m_speaker.designation','dt'=>5,'alias'=>'designation','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>"m_speaker.displays",'dt'=>6,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_speaker.id','dt'=>7,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('speaker',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('speaker');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('speaker',array("displays"=>$in['displays']));
