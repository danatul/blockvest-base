	...
	<div class="col-md-12">
    	<div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                	<label>Website</label>
                    <input type="url" class="form-control required" name="links" id="links"> 
                </div>
                <div class="form-group">
                	<label>Email</label>
...
var BaseTableDatatables = function() {
    ...
    var initDataTableFull = function() {
        table_ajax = $('.js-dataTable-full').dataTable({
			...
			"serverSide": true, 
			"ajax": {
					url :"<?=site_url('authority/getlist')?>",
					type : "GET",
					data : function(d){
...
$(document).ready(function(){
	...
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url('authority/save')?>',formdata);
	...
	$(".js-dataTable-full").on('click','.btn-edit-sites',function(){
		...
		if(ids!="")
		{
			var req = post('<?=site_url('authority/match')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
			req.done(function(out){
				if(!out.error)
				{
					$("#id").val(out.data.id);
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('authority/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('authority/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
