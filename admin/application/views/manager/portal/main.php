                        ...
                        <div class="card-body">
                        	 <form action="" method="post" id="frm-object">
                                  <div class="col-12 ">	
                                    ...
                                    <select name="id_portal_category" id="id_portal_category" class="form-control required">
                                        <?php
                                        for($i=0;$i<count($portal_category);$i++)
                                        {
                                        ?>
                                        <option value="<?=$portal_category[$i]['id']?>"><?=$portal_category[$i]['name']?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                                   ...
                                   <!-- <div class="form-group">
                                        ...
                                        <input type="text" class="form-control required" name="icon_link" id="icon_link"> 
                                    </div>
                                    -->
                                     <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label>Rss</label>
                                        <input type="url" class="form-control required" name="rss_link" id="rss_link"> 
                                    </div>
                              ...
                              <table class="table table-bordered table-striped js-dataTable-full">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-xs" style="width: 50%;">Icon</th>
                                        <th class="hidden-xs" style="width: 50%;">Rss Link</th>
...
$(document).ready(function(){
	...
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url('portal/save')?>',obj);
	...
	$(".js-dataTable-full").on('click','.btn-edit-sites',function(){
		...
		if(ids!="")
		{
			var req = post('<?=site_url('portal/match')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
			req.done(function(out){
				if(!out.error)
				{
					$("#id").val(out.data.id);
					$("#name").val(out.data.name);
					$("#rss_link").val(out.data.rss_link);
					//$("#icon_link").val(out.data.icon_link);
					$("#img-container").val(out.data.icon_link);
					$("#id_portal_category").val(out.data.id_portal_category).trigger('change');
				}
				else
				{
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('portal/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('portal/delete-logo')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('portal/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
