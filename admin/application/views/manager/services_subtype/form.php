                                        ...
                                        <select name="id_services_type" id="id_services_type" class="form-control required select2">
                                           <?php
										   	for($i=0;$i<count($arr);$i++)
											{
												...
												if(isset($data['id']))
												{
													if($data['id_services_type']==$arr[$i]['id'])
													{
										   ...
										   ?>
                                           		<option value="<?=$arr[$i]['id']?>" <?=$selected?>><?=$arr[$i]['name']?></option>
                                           <?php
											}
										   ?>
                                        </select>
...
$(document).ready(function(){
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url("services-subtype/save")?>',data);
			req.done(function(out){
				if(!out.error)
				{
					smart_success_box(out.message,'#frm-object .card-body');
					document.location.href="<?=site_url('services-subtype')?>";
				}
				else
				{
