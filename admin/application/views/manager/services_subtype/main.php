                    ...
                    <div class="card">
                        ...
                        List Services Sub
                         <span class="pull-right">
                          	 ...
                          	 <a   href="<?=site_url('services-subtype/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
                      </div>
                      <div class="card-body">
...
<script>
var table_ajax;
var curls = "<?=site_url('services-subtype/getlist')?>?baru=1";
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('services-subtype/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('services-subtype/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
