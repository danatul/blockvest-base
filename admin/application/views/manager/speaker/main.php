                        ...
                        <div class="card-body">
                        	 <form action="" method="post" id="frm-object">
                                  <div class="col-12 ">	
                                    ...
                                    <select name="id_speaker_category" id="id_speaker_category" class="form-control required">
                                        <?php
                                        for($i=0;$i<count($speaker_category);$i++)
                                        {
                                        ?>
                                        <option value="<?=$speaker_category[$i]['id']?>"><?=$speaker_category[$i]['name']?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                              ...
                              <table class="table table-bordered table-striped js-dataTable-full">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-xs" style="width: 50%;">Company</th>
                                        <th class="hidden-xs" style="width: 50%;">Nama</th>
                                        <th class="hidden-xs" style="width: 50%;">Designation</th>
...
$(document).ready(function(){
	...
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url('speaker/save')?>',obj);
	...
	$(".js-dataTable-full").on('click','.btn-edit-sites',function(){
		...
		if(ids!="")
		{
			var req = post('<?=site_url('speaker/match')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
			req.done(function(out){
				if(!out.error)
				{
					$("#id").val(out.data.id);
					$("#nama").val(out.data.nama);
					$("#company").val(out.data.company);
					...
					$("#id_speaker_category").val(out.data.id_speaker_category).trigger('change');
				}
				else
				{
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('speaker/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('speaker/delete-logo')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('speaker/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
