                        	 ...
                        	 <form action="" method="post" id="frm-object">
                                  <div class="col-12 ">	
                                     ...
                                     <div class="form-group">
                                        ...
                                        <select name="id_company_category" id="id_company_category" class="form-control select2 required">
                                            <?php
                                            for($i=0;$i<count($company_category);$i++)
                                            {
                                            ?>
                                            <option value="<?=$company_category[$i]['id']?>"><?=$company_category[$i]['name']?></option>
                                            <?php
                                            }
                                            ?>
                                        </select>
                                    ...
                                    */
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="url" class="form-control" name="smartcontract" id="smartcontract"> 
                                    </div>
                                    <div class="form-group">
                              ...
                              <table class="table table-bordered table-striped js-dataTable-full">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-xs" style="width: 50%;">Telegram</th>
                                        <th class="hidden-xs" style="width: 50%;">PIC</th>
                                        <th class="hidden-xs" style="width: 10%;">Website</th>  
...
$(document).ready(function(){
	...
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url('company/save')?>',obj);
	...
	$(".js-dataTable-full").on('click','.btn-edit-sites',function(){
		...
		if(ids!="")
		{
			var req = post('<?=site_url('company/match')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
			req.done(function(out){
				if(!out.error)
				{
					$("#id").val(out.data.id);
					$("#name").val(out.data.name);
					$("#pic").val(out.data.pic);
					$("#website").val(out.data.website);
					$("#media_link").val(out.data.media_link);
					$("#telegram").val(out.data.telegram);
					$("#description").val(out.data.description);
					 
					$("#img-container").val(out.data.images);
					$("#id_company_category").val(out.data.id_company_category).trigger('change');
					
					$("#media_link").val(out.data.media_link);
					$("#smartcontract").val(out.data.smartcontract);
					
					$("#based").val(out.data.based);
					$("#designation").val(out.data.designation);
					$("#linkedin").val(out.data.linkedin);
					$("#twitter").val(out.data.twitter); 
					$("#instagram").val(out.data.instagram); 
					$("#facebook").val(out.data.facebook); 
					$("#tiktok").val(out.data.tiktok); 
					$("#discord").val(out.data.discord); 
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('company/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('company/delete-logo')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('company/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
