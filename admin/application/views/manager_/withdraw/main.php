  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group  hashed xhide">
        	<label>Txn Hash To Customer</label>
            <input type="text" class="form-control  "   name="tx_hash" id="tx_hash" placeholder="HASH Payment" />
        </div>  
      </div>
...
<script>
var table_ajax;
var curls = "<?=site_url('withdraw/getlist')?>";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('withdraw/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('withdraw/status')?>',$("#frm-status").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
	...
	$("#frm-tx").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('withdraw/updatetx')?>',$("#frm-tx").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
	...
	$(".js-dataTable-full").on('click','.btn-resend-notif',function(){
		var ids = $(this).data('ref');
		if(ids!="")
		{
			bootbox.confirm({
				title: "Notif Resend this?",
				message: "Do you want to Notif Resend this.",
				buttons: {
					cancel: {
						label: '<i class="fa fa-times"></i> Cancel'
					},
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('withdraw/resend_notif')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
