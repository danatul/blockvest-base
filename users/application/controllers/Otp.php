...
class Otp extends Wa_Controller {
	...
	public function newsign()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$in = $this->input->post();
			$id = user_balance('id');
			$customer = $this->db->where('id',$id)->get('v_customer')->row_array();
			if(!isset($customer['id']))
			{
				json(array('error'=>true,'message'=>'Invalid Customer','security'=>token()));
				return;
			}
			...
			if($in['wa_code']==$wa_code)
			{
				$id = user_balance('id');
				$this->db->trans_begin();
				$this->db->where('id',$id)->update('customer',array("wa_code"=>$wa_code,"phone_verification"=>1,'status'=>1));
				$this->db->trans_commit();
				$this->session->unset_userdata('blockvest_customer');
				 
				$customer = $this->db->where('id',$customer['id'])->get('customer')->row_array();
				$this->session->set_userdata('blockvest_customer',$customer);
			...
			// $data = $smspintar->send('6289517704296',$wa_code);
			 ...
			 if(isset($data['status']))
			 {
				if($data['status']=="000"  || $data['status']=="001"|| $data['status']=="002")
				{
					...
					json(array('error'=>false,'message'=>'Proccess','security'=>token(),"wa_code"=>$wa_code));
					return;
				}
			 }
		}
