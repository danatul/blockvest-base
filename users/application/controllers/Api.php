...
class Api extends CI_Controller {
	...
	public function forgot()
	{
		...
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if($check->num_rows()==0)
			{
				json(array('error'=>true,'message'=>'Phone Not Exist','security'=>token()));
				return;	
			}
			...
			$in['urls'] = base_url()."stats/forgot/".$in['wa_code'];
			$this->session->set_userdata('blockvest_forgot',$in);	
			// send_mail_forgot($in);
			json(array('error'=>false,'message'=>'Proses','security'=>token(),"urls"=>$in['urls']));
			return;
		}
		show_404();
	}
	public function verify()
	{
		...
		$in['title'] = ""; 
		$in['tpl'] = "otp/forgotverify";
		$this->load->view('manager/otp/forgotverify',$in);
		return; 
	}
	public function forgotauth()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			 ...
			 $check = $this->db->where('wa_code',$wa_code)->get('customer')->row_array();
	...
	public function forgotin()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			$in = $this->input->post();
			$time = time();
			$check = $this->db->where('wa_code',$in['wa_code'])->get('customer');
			if($check->num_rows()==0)
			{
				json(array('error'=>true,'message'=>'Code Not Exist','security'=>token()));
				return;	
			}
			 
			$in['urls'] = base_url()."stats/forgot/".$in['wa_code'];
			json(array('error'=>false,'message'=>'Proses','urls'=>$in['urls'],'security'=>token()));
			return;
		}
		json(array('error'=>true,'message'=>'Proccess Failed','security'=>token()));
		return;
	...
	public function forgotsave()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			$in = $this->input->post();
			$time = time();
			$check = $this->db->where('email',$in['email'])->get('v_customer');
			if($check->num_rows()==0)
			{
				json(array('error'=>true,'message'=>'Akun Not Exist','security'=>token()));
				return;	
			}
		 
			$this->db->where('id',$in['id'])->update("customer",array('status'=>1,'password'=>$this->encryption->encrypt($in['password']))); 
			$this->session->set_userdata('blockvest_customer',$check->row_array());
	...
	{
		$in = $this->session->userdata('blockvest_forgot');
		send_mail_forgot($in);
	}
