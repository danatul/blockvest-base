...
{
	$CI =& get_instance();
	$ses = $CI->session->userdata('blockvest_customer');
...
function varpayment()
{
	...
	$arr = array("Waiting","successful","Failed","PENDING");
	return $arr;
}
...
{
	$CI =& get_instance();
	$ses = $CI->session->userdata('blockvest_customer_lang');
	return $ses;
...
{
	return $reward_percen;
	/*
	if($reward_percen>0)
	{
		$r = ($tt['price']>0 && $tt['total_seat']>0)?round(($tt['price']/$tt['total_seat']),2):0;
		return $r;
	}
	return 0;
	*/
...
function get_unique_seat()
{
	$CI =& get_instance();
	$CI->load->helper('string');
	$rand = random_string('numeric',6);
	$rs = $CI->db->where('pid',$rand)->get('event_seat');
	if($rs->num_rows()>0)
	{
		return get_unique_seat();
	}
	return $rand;
}
...
{
	$CI =& get_instance();
	$wallet = $CI->session->userdata('blockvest_nft_wallet');
...
function get_function_network($tipe)
{
	...
	$rs = $CI->db->where('display',1)->get('networks')->row_array(); 
	return $rs;
}
