            ...
            <div class="sign__left">
               ...
               <div class="sign__center-wrapper text-center mt-90">
                  <div class="sign__title-wrapper mb-40">
                     <h3 class="sign__title">Register To <?=setting('website_title')?></h3>
                  ...
                  <div class="sign__input-form text-center">
                     ...
                     <form action="javascript:void(0);" role="form" method="post" id="js-validation-login">
                        <div class="sign__input">
                           <input type="text" placeholder="User Name" required="required" class="required" name="full_name" id="full_name">
                           <span><i class="flaticon-user-2"></i></span>
                        </div>
                        <div class="sign__input">
                           <select name="id_partner_type" class="required" required>
	                           <option value="">-- Type --</option>
                               <?php
							   	for($i=0;$i<count($partner_type);$i++)
								{
							   ?>
                               		<option value="<?=$partner_type[$i]['id']?>"><?=$partner_type[$i]['name']?></option>
                               <?php
								}
							   ?>
                           </select>
                           <span><i class="flaticon-job"></i></span>
                        </div>
                        <div class="sign__input">
                           <input type="email" placeholder="Email Address" required="required" class="required" name="email" id="email">
                           <span><i class="flaticon-email"></i></span>
                        </div>
                        <div class="sign__input">
                           <input type="password" placeholder="Enter Password" min="8" class="required" required="required"  name="password" id="signin-password">
                           <span><i class="flaticon-password"></i></span>
                        </div>
                        <div class="sign__input">
                           <input type="password" placeholder="Confirm Password" min="8" class="required" required="required"    id="confirm-password" equalTo="#signin-password">
                           <span><i class="flaticon-password"></i></span>
                        </div>
   ...
   <script>
		$(function()
		{
			$('#js-validation-login').submit(function(){
					...
					var req = post('<?=site_url('register/save')?>',$("#js-validation-login").serialize());
					req.done(function(out){
						if(out.error==false)
						{
							smart_success_box(out.message,'.message');					
