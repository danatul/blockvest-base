...
{
	$CI =& get_instance();
	$ses = $CI->session->userdata('partner_blockvest_login');
...
function email_config()
{
	...
	$config = Array(
		...
		'smtp_port' => 465,
		'smtp_user' => 'hai@sospawn.xyz',//'hai@sospawn.xyz', 
	...
	*/
	...
	$config = Array(
		'protocol' => 'smtp',		 
		//'smtp_host' =>  'ssl://smtp.gmail.com',//'ssl://smtp.googlemail.com',//'ssl://smtp.zoho.com',
		'smtp_host' => 'ssl://smtp.googlemail.com',
		'smtp_port' => 465,
		'smtp_user' => 'blockvest.dev@gmail.com', 
		'smtp_pass' => '6Z5BWBYYU', 
		'mailtype'  => 'html', 
		'newline' => "\r",
		'wordwrap' => TRUE,
	...
	*/
	...
	$config['smtp_user'] = 'verify@blockvest.org';
	$config['_smtp_auth'] = TRUE;
	$config['smtp_pass'] = 'perubahan2023';
	...
	*/
	$config = Array(
		'protocol' => 'smtp',		'_smtp_auth' => true,		'wordwrap' => true,		'priority' =>3,
		'smtp_host' => 'ssl://mail.blockvest.org',
		'smtp_port' => 465,
		'smtp_user' => 'verification@blockvest.org',//'hai@sospawn.xyz', 
		'smtp_pass' => 'eah;ZiB8k(!V', 
		'mailtype'  => 'html', 
		'newline' => "\r\n",
		'charset'   => 'iso-8859-1'
	);
...
function get_unique_customer_code()
{
	...
	if($rs->num_rows()>0)
	{
		return get_unique_customer_code();
	}
	return $rand;
}  
function get_unique_partner_code()
{
	$CI =& get_instance();
	$CI->load->helper('string');
	$rand = random_string('numeric',8);
	$rs = $CI->db->where('activation_code',$rand)->get('partner');
	if($rs->num_rows()>0)
	{
		return get_unique_partner_code();
	}
	return $rand;
}  
function get_unique_order_presale()
{
	...
	{
		return get_unique_order_presale();
	}
	return $rand;
}
...
#status
function var_reward_event()
{
	$arr = array("Waiting","Approved","Rejected");	
	return $arr;
}
function reward_event($no=0)
{
	$xx = var_reward_event();
	return isset($xx[$no])?$xx[$no]:"";	
}
function var_claimstatus()
{
	$arr = array("Waiting","Present","Rejected");	
	return $arr;
}
function claimstatus($no=0)
{
	$xx = var_claimstatus();
	return isset($xx[$no])?$xx[$no]:"";	
} 
function var_proofstatus()
{
	$arr = array("Waiting","Approved","Rejected");	
	return $arr;
}
function proofstatus($no=0)
{
	$xx = var_proofstatus();
	return isset($xx[$no])?$xx[$no]:"";	
} 