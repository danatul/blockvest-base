...
class Api extends CI_Controller {
	...
	public function forgot()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->where('id',$c['id'])->update("partner",array('activation_code'=>$in['activation_code'])); 
			
			$in['urls'] = site_url("stats/forgot/".$in['activation_code']);
			$this->session->set_userdata('partner_blockvest_forgot',$in);	
			
			 send_mail_forgot($in);
			json(array('error'=>false,'message'=>'Check Your Email','security'=>token()));
			return;
		}
		show_404();
	}
	public function forgotsave()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->where('id',$in['id'])->update("partner",array('password'=>$this->encryption->encrypt($in['password']),'activation_code'=>'')); 
			$this->session->set_userdata('partner_blockvest_login',$check->row_array());
	...
	{
		$in = $this->session->userdata('partner_blockvest_forgot');
		send_mail_forgot($in);
	}
